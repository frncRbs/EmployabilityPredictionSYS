...
def login_registerIT_view():
    ...
    return render_template("IT/login_IT.html", curriculum_input=curriculum_input)

@_route_it.route('/register_IT', methods=['GET'])
def register_IT():
    auth_user=current_user
    curriculum_input = db.session.query(CurriculumResult).all()
    if auth_user.is_authenticated:
        if auth_user.user_type == 1 and auth_user.department == "Information Technology":
            ...
            return redirect(url_for('_auth.index'))
    return render_template("IT/register_IT.html", curriculum_input=curriculum_input)
